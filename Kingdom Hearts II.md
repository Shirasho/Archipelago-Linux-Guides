...
6. Extract the latest OpenKH Mod Manager into the `ProgramData` directory of the Kingdom Hearts 2 Wine prefix: `PATH/TO/steamapps/compatdata/2552430/pfx/drive_c/ProgramData/OpenKH`.
7. Add OpenKH as a non-Steam game to Steam. If extracted to the location in the previous step it would be located at `PATH/TO/steamapps/compatdata/2552430/pfx/drive_c/ProgramData/OpenKH/OpenKh.Tools.ModsManager.exe`.
8. Open the properties of the application. In the Compatibility tab change the compatibility tool to `Proton Experimental`.
9. In the Shortcut tab enter `STEAM_COMPAT_DATA_PATH="PATH/TO/steamapps/compatdata/2552430" %command%` into the launch options.
10. Run OpenKH in Steam and follow the [instructions](https://tommadness.github.io/KH2Randomizer/setup/Panacea-ModLoader/) for configuring the mod loader.
...
11. Install the mods listed in the [required software](https://archipelago.gg/tutorial/Kingdom%20Hearts%202/setup_en#required-software) section of the Kingdom Hearts II Archipelago page.
...
# Running the game
There are two different approaches to running the game - using a bash script or using [Protonhax](https://github.com/jcnils/protonhax). The recommended approach is to use a modified version of the included [Kingdom Hearts 2 bash script](./scripts/KH2.sh).
...
## Protonhax
_Protonhax is currently untested for this game since the shell script works so well._
